...
func handlePullRequestAutoMerge(pullID int64, sha string) {
	...
	if scheduledPRM.DeleteBranch {
		deleted, err := repo_service.DeleteBranchIfUnused(ctx, pr, baseGitRepo, doer)
		if err != nil {
			if deleted {
				log.Info("%-v branch deleted but got error during comment creation: %v", pr, err)
			} else {
				log.Error("%-v repo_service.DeleteBranchIfUnused: %v", pr, err)
			}
		}
	}
}
