...
func processPersonInboxCreate(ctx *context_service.APIContext, activity *ap.Activity) {
	// This, and the other two instances of nolint here are to disable the
	// "struct literal uses unkeyed fields" check. In all three cases, this is
	// an embedded type, we *want to* use unkeyed fields.
	createAct := fm.ForgeUserActivity{*activity} //nolint
	...
	userActivity := fm.ForgeUserActivityNote{*a} //nolint
	act := fm.ForgeUserActivity{*activity}       //nolint
